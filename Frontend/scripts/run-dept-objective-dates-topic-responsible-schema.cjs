...
async function main() {
  ...
  const scriptPath = path.join(__dirname, '../database/dept-objective-dates-topic-responsible.sql');
  await runSqlScript(pool, scriptPath, 'dept-objective-dates-topic-responsible.sql');
  await pool.close();
  console.log('\nDone.');
}
