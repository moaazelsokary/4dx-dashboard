...
app.post('/api/wig/department-objectives/update-order', async (req, res) => {
  try {
    ...
    // Check if sort_order column exists, if not, update without it
    ...
    if (!columnExists) {
      return res.status(400).json({ 
        error: 'sort_order column does not exist. Please run the migration: npm run migrate-order-column' 
      });
    }
    ...
    console.error('[update-order] Stack:', error.stack);
...
app.get('/api/wig/kpi-breakdown/:kpi', async (req, res) => {
  try {
    ...
    const breakdownSource = req.query.breakdown_source || req.query.breakdownSource || 'bau';
    const { computeKPIBreakdown } = require('./netlify/functions/utils/kpi-breakdown.cjs');
    const payload = await computeKPIBreakdown(pool, sql, kpi, breakdownSource);
    res.json(payload);
  } catch (error) {
