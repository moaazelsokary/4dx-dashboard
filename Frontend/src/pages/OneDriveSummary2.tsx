...
const OneDriveSummary2: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold mb-4">OneDrive Summary 2</h1>
      {loading && <div>Loading...</div>}
      {error && <div className="text-red-500">{error}</div>}
      {!loading && !error && data && (
        <div>
          {/* TODO: Add advanced summary visualizations here using data from all four tabs */}
          <pre className="bg-muted p-4 rounded text-xs overflow-x-auto">{JSON.stringify(data, null, 2)}</pre>
        </div>
      )}
    </div>
  );
...
export default OneDriveSummary2; 